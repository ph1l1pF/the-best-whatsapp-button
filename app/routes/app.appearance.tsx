...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  await prismaClient.whatsappSettings.upsert({
    where: { shop: session.shop },
    update: { showClose, buttonSize, buttonLabel, showDelaySeconds },
    create: {
      shop: session.shop,
      showClose,
      buttonSize,
...
export default function AppearanceSettings() {
  const { locale, showClose, buttonSize, buttonLabel, showDelaySeconds } =
  ...
  const previewLabel =
    draftButtonLabel || getDefaultButtonLabel(locale);

  return (
    <s-page heading={t(locale, "appearancePageTitle")}>
      <s-section heading={t(locale, "appearanceTitle")}>
        <s-paragraph>{t(locale, "appearanceSectionDescription")}</s-paragraph>
